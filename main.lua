...
local wrapIm = love.graphics.newImage("assets/testIm2.png")
...
function state:screenToCanvasCoords(sx, sy)
    local px, py =
        math.fmod(
            sx / self.scaleFactor - state.offsetX - state.curOffsetX - testCanvas:getWidth() / 2 -
            love.graphics.getWidth() / 2,
            testCanvas:getWidth()),
        math.fmod(
            sy / self.scaleFactor - state.offsetY - state.curOffsetY - testCanvas:getHeight() / 2 -
...
function love.mousemoved(x, y, dx, dy, istouch)
    ...
    if state.arrowSrc then
        local px, py = state:screenToCanvasCoords(love.mouse.getPosition())
...
function love.wheelmoved(dx, dy)
    ...
    state.scaleFactor = 2 ^ state.scaleLevel
...
function love.draw()
    ...
    love.graphics.rectangle("line", 0, 0, testCanvas:getDimensions())
    if state.arrowSrc then
        for dx = -1, 1 do
            for dy = -1, 1 do
                love.graphics.push()
